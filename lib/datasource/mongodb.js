...
class MongoDb extends Datasource {
   ...
   * @param {object} options.paginationPageSize
   * @param {object} options.offset
   * @returns {array} docs - query result as js array
   */
  async queryOverMongodb ({ paginationPageSize, offset }) {
   ...
   * @param {number} options.paginationPageSize - entries per page (default unlimited)
   * @param {date} options.modifiedAfter - query only after this date
   */
  buildMongoSshCommand ({ modifiedAfter, offset, paginationPageSize }) {
    const connectionData = this.location.match(/(.*)\/(.*)\/(.*)$/)
    const dbName = connectionData[2]
    ...
    const limitString = paginationPageSize ? `.limit(${paginationPageSize})` : ''
   ...
   *
   * @param {object} options
   * @param {number} options.paginationPageSize
   * @param {number} options.offset
   * @param {date} options.modifiedAfter
   * @returns {array} docs - query result as js array
   */
  async queryOverSsh ({ modifiedAfter, paginationPageSize, offset }) {
   ...
   * @returns {array} docs - query result as js array
   */
  async queryDatasource ({ modifiedAfter, paginationPageSize, offset } = {}) {
    if (this.location.startsWith('mongodb://')) {
      return this.queryOverMongodb({
        modifiedAfter,
        paginationPageSize,
        offset
      })
    } else if (this.location.startsWith('ssh://')) {
      return this.queryOverSsh({
        modifiedAfter,
        paginationPageSize,
        offset
      })
