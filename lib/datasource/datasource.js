...
class Datasource {
   ...
   * @param {date} options.modifiedAfter - sync everything after specified: modified date
   * @param {number} options.limit - entries per page
   * @returns {object} syncResult
   */
  async syncDatasource ({ modifiedAfter } = {}) {
    ...
    do {
      debug(
        'syncing %s entries %s to %s',
        this.name,
        offset,
        (offset + 1) * this.paginationPageSize || 'all'
      )
      const page = await this.queryDatasource({
        modifiedAfter,
        paginationPageSize: this.paginationPageSize,
        paginationLimitParam: this.paginationLimitParam,
        paginationOffsetParam: this.paginationOffsetParam,
      ...
      resultLength = this.paginationPageSize ? result.length : 0
      offset++
    } while (resultLength > 0)

    const finished = new Date()
    debug(
      'finished syncing %s in %d seconds',
      this.name,
