...
class Datasource {
   ...
   */
  async paginatedSyncOverDatasource ({modifiedAfter, Table}) {
    let resultLength = 0
    let offset = 0
    const result = []
  ...
  async syncDatasource ({ completeSync } = { completeSync: false }) {
    ...
    const result = await this.paginatedSyncOverDatasource({modifiedAfter, Table})
