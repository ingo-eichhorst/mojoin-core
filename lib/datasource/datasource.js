...
const { checkCache } = require('../utils')
...
class Datasource {
  ...
  validate () {
    if (!this.schema || typeof this.schema !== 'object') {
      throw new Error('no schema for input validation provided')
    }
  ...
  /**
   * Brings remote datasource and cache in the same state
   *
   * @param {object} Table - Sequelize table class
   * @param {number} limit - items per page
   */
  async syncPaginated (Table, limit) {
    let resultLength = 0
    let offset = 0
    const result = []
    do {
  ...
  async syncDatasource ({ modified, limit } = {}) {
    ...
    const result = await this.syncPaginated(Table, limit)
