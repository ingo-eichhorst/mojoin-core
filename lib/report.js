...
class Report {
  ...
  /**
   * Find all datasources that are included in the query
   *
   * @param {object} query
   * @returns {array} datasources - list of datasources as string
   */
  getDasourcesFromQuery (query) {
    ...
    datasourcesInQuery.push(query.table)
    return datasourcesInQuery
  }
  /**
  ...
  async syncInvolvedDatasources () {
    const datasourcesInQuery = this.getDasourcesFromQuery(this.query)
