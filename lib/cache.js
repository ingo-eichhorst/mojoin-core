...
class Cache {
   ...
   */
  transformToSequelizeQuery (table, sourceQuery) {
    const query = JSON.parse(JSON.stringify(sourceQuery))
    query.include = query.include.map(include => {
      this.tables[table].belongsTo(this.tables[include.model], {foreignKey: include.foreignKey})
      return {
   ...
   *
   * @param {string} table - name to identify the table
  ...
  async query (table, query) {
    ...
    const sequalizeQuery = this.transformToSequelizeQuery(table, query)
    const result = await this.tables[table].findAll(sequalizeQuery)
