...
class Cache {
   ...
   * @constructor
   * @param {object} config
   * @param {string} type - 'mysql'|'sqlite'|'postgres'|'mssql'
   * @param {string} location - url or path to databse
   * @param {string} username - login credentials
   * @param {string} password - login credentials
   */
  constructor ({type, location, username, password}) {
    this.databaseName = 'mojoin-cache'
    this.initSequelize({type, location, username, password})
    this.type = type
